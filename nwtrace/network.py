...
# NOTE: WIP refactor to clean up codebase
...
class Network:
    ...
    def __init__(
        self,
        segments: str | Path | gpd.GeoDataFrame | pd.DataFrame,
        nodes: str | Path | gpd.GeoDataFrame | pd.DataFrame | None,
        id_field: str,
        node_id_field: str | None,
        ...
        """
        Initializes a Network object instance

        Parameters
        ----------
        segments : str | Path | gpd.GeoDataFrame | pd.DataFrame
            The datast of segments representing the network, which may contain segment-node connection information.
            Or, a filepath to the file containing the node-line connections (must be openable by pandas or geopandas, may include spatial data)
        nodes: str | Path | gpd.GeoDataFrame | pd.DataFrame | None
            The dataset of nodes that connect the segments in the segment dataset, or the path to a filestructure that can be opened by pandas or geopandas.. 
            If none, no nodes will be used and segments will be connected by false/generated nodes (NOTE: not implemented yet - generating nodes).
        ...
        # load vectors
        self.segments = self._load_vector(segments)

        if not isinstance(self.segments, gpd.GeoDataFrame):
            # NOTE: Change to a better error raising system
            print(f"Warning: {segments} has no associated geometry")

        # has_nodes is true unless nodes dataset is None
        self.has_nodes = True   
        self.nodes = self._load_vector(nodes)
        ...
        if id_field not in self.segments.columns:
            raise ValueError(
                f"'{id_field}' is not present in {segments}"
            )
        
        self.id_field = id_field

        # both fields must not be None and exist in the dataset for the network to be considered connected already
        if upstream_field is None or downstream_field is None:
            self.has_connections = False
        elif upstream_field not in self.segments.columns or downstream_field not in self.segments.columns:
            raise ValueError(
                f"'{upstream_field}' or '{downstream_field}' is not in '{segments}' dataset."
            )
        ...
        # LATER NOTE: Yes I am doing that here, initialization will consider the structure of the data first
    ...
    # NOTE: may ultimately be better practice to not have this
    def _load_vector(
        self,
        vector: str | Path | pd.DataFrame | gpd.GeoDataFrame | None
    ) -> pd.DataFrame | gpd.GeoDataFrame | None:

        # load segments file using pandas or assign
        if isinstance(vector, (str, Path)):
            # Set up paths, throws FileNotFoundError if issues occur
            if self.verbose: print(f"Loading network file: `{vector}`...")
            vector_gdf = gpd.read_file(vector)
        elif isinstance(vector, pd.DataFrame) or isinstance(vector, gpd.GeoDataFrame):
            vector_gdf = vector
        else:
            raise ValueError(
                f"{vector} must be a valid file path or a DataFrame/GeoDataFrame."
            )
            return None
    ...
    def _create_lookup(self) -> tuple[dict, dict]:
        ...
        df_grav_ups = self.segments[[self.id_field, self.upstream_field]].rename(columns={self.upstream_field: 'node_id', self.id_field: 'segment_id'})
        df_grav_dwns = self.segments[[self.id_field, self.downstream_field]].rename(columns={self.downstream_field: 'node_id', self.id_field: 'segment_id'})
    ...
    def _create_directional_lookup(
        ...
        df_grav_dir = self.segments.rename(columns={self.upstream_field: 'from', self.downstream_field: 'to', self.id_field: 'segment_id'})
